...
video_path = "/private/home/keli22/datasets/PoseTrack21/images"
...
def parse_args():
    ...
    parser.add_argument('--trainval', type=str, required=True)
...
def main():
    ...
    for vidx, video_name in enumerate(os.listdir(osp.join(video_path, args.trainval))):
        # ipdb.set_trace()
        frames = list(os.listdir(osp.join(video_path, args.trainval, video_name)))
        if DEBUG:
            frames = frames[:2]
        ...
        for frame_file_name in frames:
            try:
                ...
                print(osp.join(video_path, args.trainval, video_name), frame_file_name)
            frame_level_info = {
                "frame_idx": frame_id, 
                "image_location": f"images/{args.trainval}/{video_name}/{frame_file_name}", 
                "img": f"1{vid_id}{frame_id:04}"
            }
            img = extract_image(osp.join(video_path, args.trainval, video_name, frame_file_name))
    ...
    mmcv.dump(tbd, osp.join("/private/home/keli22/CorrTrack/baselines/data/detections", f"PoseTrack21_swin_bb_thres_{args.det_score_thr}_{args.trainval}.json"))
